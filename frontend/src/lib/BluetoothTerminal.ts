...
export class BluetoothTerminal {
  ...
  _serviceUuid: number | string;
  _characteristicUuid: number | string;
  _receiveSeparator: string;
  _sendSeparator: string;
  _boundHandleDisconnection: (event: Event) => void;
  _boundHandleCharacteristicValueChanged: (event: Event) => void;
  ...
  constructor(serviceUuid: (number | string) = 0xFFE0, characteristicUuid: (number | string) = 0xFFE1,
    ...
    this._receiveBuffer = '';
    this._maxCharacteristicValueLength = 40;
    this._device = null;
  ...
  send(data: string): Promise<void> {
    ...
    let promise = this._writeToCharacteristic(characteristic, chunks[0]);
   ...
   * @private
   */
  _connectToDevice(device: BluetoothDevice | null): Promise<void> {
   ...
   * @private
   */
  _disconnectFromDevice(device: BluetoothDevice) {
    if (!device) {
      return;
    }
   ...
   * @private
   */
  _requestBluetoothDevice(): Promise<BluetoothDevice> {
   ...
   * @private
   */
  async _connectDeviceAndCacheCharacteristic(device: BluetoothDevice): Promise<BluetoothRemoteGATTCharacteristic | undefined> {
   ...
   * @private
   */
  _startNotifications(characteristic: BluetoothRemoteGATTCharacteristic): Promise<void> {
   ...
   * @private
   */
  _stopNotifications(characteristic: BluetoothRemoteGATTCharacteristic): Promise<void> {
   ...
   * @private
   */
  _handleDisconnection(event: Event) {
   ...
   * @private
   */
  _handleCharacteristicValueChanged(event: Event) {
   ...
   * @private
   */
  _writeToCharacteristic(characteristic: BluetoothRemoteGATTCharacteristic, data: string): Promise<void> {
   ...
   * @private
   */
  static _splitByLength(string: string, length: number): string[] {
