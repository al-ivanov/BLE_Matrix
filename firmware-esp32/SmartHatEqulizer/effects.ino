...
void brightnessRoutine() {
  if (brightnessDirection) {
    breathBrightness += 2;
    if (breathBrightness > currentBridgest - 2) {
      breathBrightness = currentBridgest;
...
void rainbowRoutine() {
  ...
  for (byte i = 0; i < kMatrixWidth; i++) {
    CRGB thisColor = CHSV((byte)(hue + i * float(255 / kMatrixWidth)), 255, 255);
...
void rainbowDiagonalRoutine() {
  ...
  for (byte x = 0; x < kMatrixWidth; x++) {
    for (byte y = 0; y < kMatrixHeight; y++) {
      CRGB thisColor = CHSV((byte)(hue + (float)(kMatrixWidth / kMatrixHeight * x + y) * (float)(255 / max(kMatrixWidth, kMatrixHeight))), 255, 255);      
...
void matrixRoutine() {
  ...
  for (byte x = 0; x < kMatrixWidth; x++) {
    // заполняем случайно верхнюю строку
    uint32_t thisColor = getPixColorXY(x, kMatrixHeight - 1);
    if (thisColor == 0)
      drawPixelXY(x, kMatrixHeight - 1, 0x00FF00 * (random(0, 10) == 0));
    else if (thisColor < 0x002000)
      drawPixelXY(x, kMatrixHeight - 1, 0);
    else
      drawPixelXY(x, kMatrixHeight - 1, thisColor - 0x002000);
  }
...
void ballsRoutine() {
  if (loadingFlag) {
    ...
    for (byte j = 0; j < BALLS_AMOUNT; j++) {
      ...
      coord[j][0] = kMatrixWidth / 2 * 10;
      random(0, 2) ? sign = 1 : sign = -1;
      vector[j][0] = random(4, 15) * sign;
      coord[j][1] = kMatrixHeight / 2 * 10;
      random(0, 2) ? sign = 1 : sign = -1;
  ...
  for (byte j = 0; j < BALLS_AMOUNT; j++) {
    ...
    if (coord[j][0] > (kMatrixWidth - 1) * 10) {
      coord[j][0] = (kMatrixWidth - 1) * 10;
      vector[j][0] = -vector[j][0];
    }
    if (coord[j][1] > (kMatrixHeight - 1) * 10) {
      coord[j][1] = (kMatrixHeight - 1) * 10;
